...
double PolySupportContact(tVertex PolyVerts, Vec A, Vec vContactP, int *dContactIndex)
{
  int		 i=0;
  ...
  vecCopy(PolyVerts[*dContactIndex].v,vContactP);
...
void PolyMinDist(tVertex Poly1, int m1, tVertex Poly2, int m2, double VP[], int near_indx[][2], double lambda[], int *m3)
{
  bool pass;
  ...
  if ((*m3) == 0) {	     /** if *m3 == 0 use single point initialization **/
    set_size = 1;
    vecSub(Poly1[0].v,Poly2[0].v,Pk[0]);
    i_tab[0] = j_tab[0] = 0;
    }
    ...
    for (k = 0; k < (*m3); k++) {
      ...
      vecSub(Poly1[i].v,Poly2[j].v,Pk[k]);
      }
    set_size = *m3;
    }
  ...
  /** eq. (13) **/
    ...
    Gp = vecDot(Vk, Vk) + PolySupportContactDiff(Poly1, Poly2, neg_Vk, Cp, &P1_i, &P2_i);
...
bool PolyGetNewPlane(COUPLE couple)
{
  tVertex poly1, poly2;
  bool plane_exists;
  double dist, u[3], v[3], lambda[4], VP[3];
  int i, k, m1, m2;
  Vec vTmp;
  ...
  m1 = poly1->prev->vnum+1;
  for (i = 0; i < m1; i++) {
    vecAdd(poly1[i].v,couple->p->r,poly1[i].v);
    }

  m2 = poly2->prev->vnum+1;
  for (i = 0; i < m2; i++) {
    vecAdd(poly2[i].v,couple->pn->r,poly2[i].v);
    }

/** solve eq. (1) for two polytopes **/
  PolyMinDist(poly1, m1, poly2, m2, VP, couple->vert_indx, lambda, &couple->n);
  ...
  if (!EQZ(dist)) {	       /** Does a separating plane exist **/
    ...
    for (i = 0; i < couple->n; i++) {
      ...
      vecScale(poly1[k], lambda[i], vTmp);
      vecAdd(vTmp, u, u);
      
      k = couple->vert_indx[i][1];
      vecScale(poly2[k], lambda[i], vTmp);
      vecAdd(vTmp, v, v);
      }
...
bool PolyCollisionTest(COUPLE couple)
{
  tVertex poly1, poly2;
  Vec vTmp;
